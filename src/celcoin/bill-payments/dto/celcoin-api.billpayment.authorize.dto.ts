...
export class CelcoinApiBillPaymentAuthorizeDto {
    ...
    @ApiProperty({ description: 'Transaction identifier on the client system - its not necessary to include' })
    externalNSU: number;

    @IsObject()
    @ApiProperty({ description: 'Barcode object which has the barcode properties' })
    barCode: BarCodeDto;
